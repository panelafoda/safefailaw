...
add_option(menu, text, func, bool, input, input2, input3, input4)
{
    ...
    if(isdefined(bool))
    {
        ...
        bool = "^7[^2ON^7]";
        self.menu.text[menu][index] = text + " : ^7" + bool;
    }
    else
...
add_bind(menu, text, func, dvar)
{
    index = self.menu.text[menu].size;
    if(getdvar(dvar) == "[OFF]")
    self.menu.text[menu][index] = text + ": ^7[^1OFF^7]";
    else if(getdvar(dvar) == "+actionslot 1")
    self.menu.text[menu][index] = text + ": ^7[Dpad_Up]";
    else if(getdvar(dvar) == "+actionslot 2")
    self.menu.text[menu][index] = text + ": ^7[Dpad_Down]";
    else if(getdvar(dvar) == "+actionslot 3")
    self.menu.text[menu][index] = text + ": ^7[Dpad_Left]";
    else if(getdvar(dvar) == "+actionslot 4")
    self.menu.text[menu][index] = text + ": ^7[Dpad_Right]";
    else if(getdvar(dvar) == "+frag")
    self.menu.text[menu][index] = text + ": ^7[RB]";
    else if(getdvar(dvar) == "+smoke")
    self.menu.text[menu][index] = text + ": ^7[LB]";
...
getcrosshaircenter()
{
    point = bullettrace(self geteye(), self geteye() + anglestoforward(self getangles()) * 1000000, 0, self)["position"];
    return point;
}
